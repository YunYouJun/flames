...
- 完整目录固定为 9 个视觉家族、22 个基础异火 preset；帝炎作为第 1 席终局视觉，在基础席全部完成后开放，当前已开放全部 23 席。
- 同时驻留的 Three.js program 硬上限为 17 个，单席活跃 program 不超过 6 个（含承台、体积与合成），其余预算以 [基础约定](./flame-foundation.md) 为准。
...
以上为代表作而非完整公开名单。全部 23 席均已完成三维实现、正侧面与交互复查，并统一设为 `approved`；开发和生产保持一致。后续若有席位退回 `prototype`，继续使用封印／不可索引边界，并同步收回帝炎终局开放状态。
